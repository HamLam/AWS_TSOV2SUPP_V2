...
tables_path="$current_path/exome_pre_loaded_tables_Chr6_Chr7_Chr8"
echo $tables_path
...
# run_sample3_wrapper.sh
sed -e s,sample_path,"$sample_path",g < "$template_pwd/run_sample3_wrapper.sh" > "$sample_path/run_cnv_wrapper_$sample_name.sh"
...
-e s,code_path,"$current_path",g -e s,seq_db,"$seq_db",g < "$template_pwd/run_sample3.sh" > "$sample_path/run_cnv_$sample_name.sh"
...
# create_reference_g1.sql
sed -e s,control_name,"$control_name",g -e s,sample_name,"$sample_name",g < "$template_pwd/create_reference_g3.sql" > "$sample_path/create_reference_g3.sql"

# find_median.R
sed -e s,sample_path,"$sample_path",g -e s,sample_name,"$sample_name",g -e s,control_name,"$control_name",g \
-e s,scripts_location,"$scripts_location",g -e s,socket_path,"$socket_path",g -e s,dbname\=\"cnv\","$dbg3",g \
...
# create_tables_ref.R                                                                                                                                        
sed -e s,sample_path,"$sample_path",g -e s,sample_name,"$sample_name",g -e s,control_name,"$control_name",g -e s,dbname\=\"cnv\","$dbg3",g \
...
#cp $template_pwd/chr6_chr7_chr8_ordered_genes.txt $sample_path/ordered_genes.txt
echo "Searching chr6-8 genes"
mv $sample_path/ordered_genes.txt $sample_path/ordered_genes.txt.orig
perl $scripts_location/find_chr6_7_8.pl $template_pwd/TSOV2supp_genes_chr_listed $sample_path/ordered_genes.txt.orig
cp $current_path/chr6_7_8_genes_file.txt $sample_path/ordered_genes.txt
cp $current_path/chr6_7_8_genes_file.txt $sample_path/chr6_7_8_genes_file.txt
echo "chr6-8 genes searched"
...
-e s,socket_path,"$socket_path",g -e s,tables_path,"$tables_path",g -e s,dbname\=\"cnv\","$dbg3",g \
