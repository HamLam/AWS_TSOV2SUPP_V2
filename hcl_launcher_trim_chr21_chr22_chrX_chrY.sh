...
FILE=$1
current_path=$(pwd)
echo "Testing the CNV pipeline script....."
...
tables_path="$current_path/exome_pre_loaded_tables_Chr21_Chr22_ChrX_ChrY"
echo $tables_path
...
socket_path="$user_tmp/mysql/thesock"
echo "socket_path: $socket_path"

# cnv group 7 database name
dbg7=dbname\=\"cnv7\"
...
# run_sample7_wrapper.sh
sed -e s,sample_path,"$sample_path",g < "$template_pwd/run_sample7_wrapper.sh" > "$sample_path/run_cnv_wrapper_$sample_name.sh"
...
-e s,code_path,"$current_path",g -e s,seq_db,"$seq_db",g < "$template_pwd/run_sample7.sh" > "$sample_path/run_cnv_$sample_name.sh"
...
# Pre_Load_Control.sql
sed -e s,control_name,"$control_name",g < "$template_pwd/Pre_Load_Control.sql" > "$sample_path/Pre_Load_Control.sql"

# load_control.sql
sed -e s,control_name,"$control_name",g < "$template_pwd/load_control.sql" > "$sample_path/load_control.sql"

# Pre_Load_Sample.sql
sed -e s,sample_name,"$sample_name",g < "$template_pwd/Pre_Load_Sample.sql" > "$sample_path/Pre_Load_Sample.sql"
...
# create_reference_g7.sql
sed -e s,control_name,"$control_name",g -e s,sample_name,"$sample_name",g < "$template_pwd/create_reference_g7.sql" > "$sample_path/create_reference_g7.sql"

# find_median.R
sed -e s,sample_path,"$sample_path",g -e s,sample_name,"$sample_name",g -e s,control_name,"$control_name",g \
-e s,scripts_location,"$scripts_location",g -e s,socket_path,"$socket_path",g -e s,dbname\=\"cnv\","$dbg7",g \
...
# create_tables_ref.R                                                                                                                                        
sed -e s,sample_path,"$sample_path",g -e s,sample_name,"$sample_name",g -e s,control_name,"$control_name",g -e s,dbname\=\"cnv\","$dbg7",g \
...
#cp $template_pwd/chr21_chr22_chrX_chrY_ordered_genes.txt $sample_path/ordered_genes.txt
echo "Searching chr21-Y genes"
mv $sample_path/ordered_genes.txt $sample_path/ordered_genes.txt.orig
perl $scripts_location/find_chr21_22_X_Y.pl $template_pwd/TSOV2supp_genes_chr_listed $sample_path/ordered_genes.txt.orig
cp $current_path/chr21_22_X_Y_genes_file.txt $sample_path/ordered_genes.txt
cp $current_path/chr21_22_X_Y_genes_file.txt $sample_path/chr21_22_X_Y_genes_file.txt
echo "chr21-Y genes searched"
...
-e s,socket_path,"$socket_path",g -e s,tables_path,"$tables_path",g -e s,dbname\=\"cnv\","$dbg7",g \
...
# plot_genes_ordered_cnv7.py                                                                
sed -e s,sample_path,"$sample_path",g -e s,scripts_location,"$scripts_location",g \
-e s,socket_path,"$socket_path",g -e s,tables_path,"$tables_path",g \
< "$template_pwd/plot_genes_ordered_cnv7.py" > "$sample_path/plot_genes_ordered_cnv7.py"
