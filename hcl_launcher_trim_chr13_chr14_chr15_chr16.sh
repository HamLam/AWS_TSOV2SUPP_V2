...
tables_path="$current_path/exome_pre_loaded_tables_Chr13_Chr14_Chr15_Chr16"
echo $tables_path
...
# run_sample5_wrapper.sh
sed -e s,sample_path,"$sample_path",g < "$template_pwd/run_sample5_wrapper.sh" > "$sample_path/run_cnv_wrapper_$sample_name.sh"
...
-e s,code_path,"$current_path",g -e s,seq_db,"$seq_db",g < "$template_pwd/run_sample5.sh" > "$sample_path/run_cnv_$sample_name.sh"
...
# create_reference_g5.sql
sed -e s,control_name,"$control_name",g -e s,sample_name,"$sample_name",g < "$template_pwd/create_reference_g5.sql" > "$sample_path/create_reference_g5.sql"

# find_median.R
sed -e s,sample_path,"$sample_path",g -e s,sample_name,"$sample_name",g -e s,control_name,"$control_name",g \
-e s,scripts_location,"$scripts_location",g -e s,socket_path,"$socket_path",g -e s,dbname\=\"cnv\","$dbg5",g \
...
# create_tables_ref.R                                                                                                                                        
sed -e s,sample_path,"$sample_path",g -e s,sample_name,"$sample_name",g -e s,control_name,"$control_name",g -e s,dbname\=\"cnv\","$dbg5",g \
...
#cp $template_pwd/chr13_chr14_chr15_chr16_ordered_genes.txt $sample_path/ordered_genes.txt
echo "Searching chr13-16 genes"
mv $sample_path/ordered_genes.txt $sample_path/ordered_genes.txt.orig
perl $scripts_location/find_chr13_14_15_16.pl $template_pwd/TSOV2supp_genes_chr_listed $sample_path/ordered_genes.txt.orig
cp $current_path/chr13_14_15_16_genes_file.txt $sample_path/ordered_genes.txt
cp $current_path/chr13_14_15_16_genes_file.txt $sample_path/chr13_14_15_16_genes_file.txt
