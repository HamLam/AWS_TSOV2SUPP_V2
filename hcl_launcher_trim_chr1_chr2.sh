...
# run_sample1_wrapper.sh
sed -e s,sample_path,"$sample_path",g < "$template_pwd/run_sample1_wrapper.sh" > "$sample_path/run_cnv_wrapper_$sample_name.sh"
...
-e s,code_path,"$current_path",g -e s,seq_db,"$seq_db",g < "$template_pwd/run_sample1.sh" > "$sample_path/run_cnv_$sample_name.sh"
...
sed -e s,control_name,"$control_name",g -e s,sample_name,"$sample_name",g < "$template_pwd/create_reference_g1.sql" > "$sample_path/create_reference_g1.sql"

# find_median.R
sed -e s,sample_path,"$sample_path",g -e s,sample_name,"$sample_name",g -e s,control_name,"$control_name",g \
-e s,scripts_location,"$scripts_location",g -e s,socket_path,"$socket_path",g -e s,dbname\=\"cnv\","$dbg1",g \
...
# create_tables_ref.R                                                                                                                                        
sed -e s,sample_path,"$sample_path",g -e s,sample_name,"$sample_name",g -e s,control_name,"$control_name",g -e s,dbname\=\"cnv\","$dbg1",g \
...
echo "Searching chr1-2 genes"
mv $sample_path/ordered_genes.txt $sample_path/ordered_genes.txt.orig
perl $scripts_location/find_chr1_2.pl $template_pwd/TSOV2supp_genes_chr_listed $sample_path/ordered_genes.txt.orig
cp $current_path/chr1_2_genes_file.txt $sample_path/ordered_genes.txt
cp $current_path/chr1_2_genes_file.txt $sample_path/chr1_2_genes_file.txt
echo "chr1-2 genes searched"
...
-e s,socket_path,"$socket_path",g -e s,tables_path,"$tables_path",g -e s,dbname\=\"cnv\","$dbg1",g \
...
#plot_genes_ordered_cnv1.py
sed -e s,sample_path,"$sample_path",g -e s,scripts_location,"$scripts_location",g \
-e s,socket_path,"$socket_path",g -e s,tables_path,"$tables_path",g \
< "$template_pwd/plot_genes_ordered_cnv1.py" > "$sample_path/plot_genes_ordered_cnv1.py"
