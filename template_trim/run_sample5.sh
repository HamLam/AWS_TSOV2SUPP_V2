...
grep "create_reference_g5.sql" $working_dir/completed.txt > /dev/null 2>&1
if [ "$?" = "0" ]; then
    echo "create_reference_g5.sql already run"
else
    echo "Run create_reference_g5.sql"
    mysql --socket=$BASE/thesock -u root cnv5 < create_reference_g5.sql
    if [[ $? -ne 0 ]] ; then
	echo "Run create_reference_g5.sql failed" >&2
	## mysqladmin --socket=$BASE/thesock shutdown -u root
	exit 1
    else
	echo "create_reference_g5.sql" >> $working_dir/completed.txt
    fi
fi
...
if [ "$?" = "0" ]; then
    ...
    if [[ $? -ne 0 ]] ; then
	echo "Run smooth_coverage.R failed cnv5" >&2
	## mysqladmin --socket=$BASE/thesock shutdown -u root
...
if [ "$?" = "0" ]; then
    ...
    if [[ $? -ne 0 ]] ; then
	echo "Run get_three_ref.R failed cnv5" >&2
	## mysqladmin --socket=$BASE/thesock shutdown -u root
	exit 1
    else
...
if [ "$?" = "0" ]; then
 ...
 echo "Run Get_3_random_ref_genes"
  mysqldump -u root --socket=$BASE/thesock --tab=$working_dir cnv5 cnv_sample_name_over_control_name_60bp_exon_ref1_med_gene_cov
  ...
  mysqldump -u root --socket=$BASE/thesock --tab=$working_dir cnv5 cnv_sample_name_over_control_name_60bp_exon_ref2_med_gene_cov
  ...
  mysqldump -u root --socket=$BASE/thesock --tab=$working_dir cnv5 cnv_sample_name_over_control_name_60bp_exon_ref3_med_gene_cov
...
if [ "$?" = "0" ]; then
    ...
    echo "plot_script.pl"
    perl $script_path/plot_script.pl -t cnv_sample_name_tso_over_control_name_n_bowtie_bwa_ratio_gene_out -s sample_name -c cnv_sample_name_ordered_genes -k cnv_sample_name_ordered_genes -h localhost -u root -d cnv5 -o plot_genes_ordered_cnv5.py -ms $mysql_socket -a 1
    if [[ $? -ne 0 ]] ; then
...
#
grep "plot_genes_ordered_cnv5.py" $working_dir/completed.txt > /dev/null 2>&1
if [ "$?" = "0" ]; then
   echo "plot_genes_ordered_cnv5.py already run"
else
    echo "plot_genes_ordered_cnv5.py"
    #R CMD BATCH plot_genes_ordered.R
    python plot_genes_ordered_cnv5.py
    if [[ $? -ne 0 ]] ; then
	echo "Run plot_genes_ordered_cnv5.py failed" >&2
	## mysqladmin --socket=$BASE/thesock shutdown -u root
	#exit 1
    else
	echo "plot_genes_ordered_cnv5.py" >> $working_dir/completed.txt
    fi
fi
echo -n "Finished plot_genes_ordered_cnv5.py " >> $working_dir/time_check
timecheck=`(date +"%Y-%m-%d [ %H:%M:%S ]")`;
echo ${timecheck} >> $working_dir/time_check
...
if [ "$?" = "0" ]; then
    ...
    echo "move_script.pl"
    perl $script_path/move_script.pl -c cnv_sample_name_ordered_genes -p sample_result -h localhost -u root -d cnv5 -o move_plots_cnv5.sh -ms $mysql_socket
    if [[ $? -ne 0 ]] ; then
        echo "Run move_script.pl cnv5 failed" >&2
        ## mysqladmin --socket=$BASE/thesock shutdown -u root
        #exit 1
    else
        echo "move_plots.pl cnv5 ran successfully"
    fi
fi
#
...
#
grep "move_plots_cnv5.sh" $working_dir/completed.txt > /dev/null 2>&1
if [ "$?" = "0" ]; then
    echo "move_plots_cnv5.sh already run"
else
    echo "Run move_plots_cnv5.sh"
    sh move_plots_cnv5.sh
    if [[ $? -ne 0 ]] ; then
        echo "Run move_plots_cnv5.sh failed" >&2
        #exit 1
    else
    	echo "move_script.pl" >> $working_dir/completed.txt
        echo "move_plots_cnv5.sh" >> $working_dir/completed.txt
    fi
fi
