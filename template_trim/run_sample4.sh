...
grep "create_reference_g4.sql" $working_dir/completed.txt > /dev/null 2>&1
if [ "$?" = "0" ]; then
    echo "create_reference_g4.sql already run"
else
    echo "Run create_reference_g4.sql"
    mysql --socket=$BASE/thesock -u root cnv4 < create_reference_g4.sql
    if [[ $? -ne 0 ]] ; then
	echo "Run create_reference_g4.sql failed" >&2
	## mysqladmin --socket=$BASE/thesock shutdown -u root
	exit 1
    else
	echo "create_reference_g4.sql" >> $working_dir/completed.txt
    fi
fi
...
if [ "$?" = "0" ]; then
    ...
    if [[ $? -ne 0 ]] ; then
	echo "Run smooth_coverage.R failed cnv4" >&2
	## mysqladmin --socket=$BASE/thesock shutdown -u root
...
if [ "$?" = "0" ]; then
 ...
 echo "Run Get_3_random_ref_genes"
  mysqldump -u root --socket=$BASE/thesock --tab=$working_dir cnv4 cnv_sample_name_over_control_name_60bp_exon_ref1_med_gene_cov
  ...
  mysqldump -u root --socket=$BASE/thesock --tab=$working_dir cnv4 cnv_sample_name_over_control_name_60bp_exon_ref2_med_gene_cov
  ...
  mysqldump -u root --socket=$BASE/thesock --tab=$working_dir cnv4 cnv_sample_name_over_control_name_60bp_exon_ref3_med_gene_cov
...
if [ "$?" = "0" ]; then
    echo "plot_script.pl cnv4 already run"
else
    echo "plot_script.pl"
    perl $script_path/plot_script.pl -t cnv_sample_name_tso_over_control_name_n_bowtie_bwa_ratio_gene_out -s sample_name -c cnv_sample_name_ordered_genes -k cnv_sample_name_ordered_genes -h localhost -u root -d cnv4 -o plot_genes_ordered_cnv4.py -ms $mysql_socket -a 1
    if [[ $? -ne 0 ]] ; then
	echo "Run plot_script.pl failed" >&2
	## mysqladmin --socket=$BASE/thesock shutdown -u root
	#exit 1
    else
	echo "plot_script.pl cnv4" >> $working_dir/completed.txt
    fi
fi
#
grep "plot_genes_ordered_cnv4.py" $working_dir/completed.txt > /dev/null 2>&1
if [ "$?" = "0" ]; then
    echo "plot_genes_ordered_cnv4.py already run"
else
    echo "plot_genes_ordered_cnv4.py"
    #R CMD BATCH plot_genes_ordered.R
    python plot_genes_ordered_cnv4.py
    if [[ $? -ne 0 ]] ; then
	echo "Run plot_genes_ordered_cnv4.py failed" >&2
	## mysqladmin --socket=$BASE/thesock shutdown -u root
	#exit 1
    else
	echo "plot_genes_ordered_cnv4.py" >> $working_dir/completed.txt
    fi
fi
...
#   sync
#   if [[ $? -ne 0 ]] ; then
#	echo "Run get_ordered_genes.sql cnv4 failed" >&2
	## mysqladmin --socket=$BASE/thesock shutdown -u root
	#exit 1
 #   else
#	echo "get_ordered_genes.sql cnv4" >> $working_dir/completed.txt
...
#then
 #   cp  sample_name_cnv_calls_on_ordered_genes_$_now.txt sample_result
    #callsize=`du -sh sample_name_cnv_calls_on_ordered_genes_$_now.txt | cut -f1`
...
if [ "$?" = "0" ]; then
    echo "move_script.pl cnv4 already run"
else
    echo "move_script.pl cnv4"
    perl $script_path/move_script.pl -c cnv_sample_name_ordered_genes -p sample_result -h localhost -u root -d cnv4 -o move_plots_cnv4.sh -ms $mysql_socket
    if [[ $? -ne 0 ]] ; then
        echo "Run move_script.pl failed cnv4" >&2
        ## mysqladmin --socket=$BASE/thesock shutdown -u root
        #exit 1
    else
        echo "move_plots.pl cnv4 ran successfully"
    fi
fi
#
...
#
grep "move_plots_cnv4.sh" $working_dir/completed.txt > /dev/null 2>&1
if [ "$?" = "0" ]; then
    echo "move_plots_cnv4.sh already run"
else
    echo "Run move_plots.sh"
    sh move_plots_cnv4.sh
    if [[ $? -ne 0 ]] ; then
        echo "Run move_plots_cnv4.sh failed" >&2
        #exit 1
    else
    	echo "move_script.pl" >> $working_dir/completed.txt
        echo "move_plots_cnv4.sh" >> $working_dir/completed.txt
    fi
fi
