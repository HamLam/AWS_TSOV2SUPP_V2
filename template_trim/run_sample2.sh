...
grep "create_reference_g2.sql" $working_dir/completed.txt > /dev/null 2>&1
if [ "$?" = "0" ]; then
    echo "create_reference_g2.sql already run"
else
    echo "Run create_reference_g2.sql"
    mysql --socket=$BASE/thesock -u root cnv2 < create_reference_g2.sql
    if [[ $? -ne 0 ]] ; then
	echo "Run create_reference_g2.sql failed" >&2
	## mysqladmin --socket=$BASE/thesock shutdown -u root
	exit 1
    else
       # echo "create_reference_g2.sql done"
	echo "create_reference_g2.sql" >> $working_dir/completed.txt
    fi
fi
...
if [ "$?" = "0" ]; then
    ...
    if [[ $? -ne 0 ]] ; then
	echo "Run smooth_coverage.R failed cnv2" >&2
	## mysqladmin --socket=$BASE/thesock shutdown -u root
...
if [ "$?" = "0" ]; then
 ...
 echo "Run Get_3_random_ref_genes"
  mysqldump -u root --socket=$BASE/thesock --tab=$working_dir cnv2 cnv_sample_name_over_control_name_60bp_exon_ref1_med_gene_cov
  ...
  mysqldump -u root --socket=$BASE/thesock --tab=$working_dir cnv2 cnv_sample_name_over_control_name_60bp_exon_ref2_med_gene_cov
  ...
  mysqldump -u root --socket=$BASE/thesock --tab=$working_dir cnv2 cnv_sample_name_over_control_name_60bp_exon_ref3_med_gene_cov
...
then
	...
	head -n1 $working_dir/completed.txt > $working_dir/completed.txt.tmp
	mv $working_dir/completed.txt.tmp $working_dir/completed.txt
        echo "Empty cnv_calls file..RESTARTING cnv2"
	rm $working_dir/*_t
	exit 1
fi
