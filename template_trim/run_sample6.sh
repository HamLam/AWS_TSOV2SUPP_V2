...
grep "create_reference_g6.sql" $working_dir/completed.txt > /dev/null 2>&1
if [ "$?" = "0" ]; then
    echo "create_reference_g6.sql already run"
else
    echo "Run create_reference_g6.sql"
    mysql --socket=$BASE/thesock -u root cnv6 < create_reference_g6.sql
    if [[ $? -ne 0 ]] ; then
	echo "Run create_reference_g6.sql failed" >&2
	## mysqladmin --socket=$BASE/thesock shutdown -u root
	exit 1
    else
	echo "create_reference_g6.sql" >> $working_dir/completed.txt
    fi
fi

echo -n "Finished create_reference_g6.sql " >> $working_dir/time_check
timecheck=`(date +"%Y-%m-%d [ %H:%M:%S ]")`;
echo ${timecheck} >> $working_dir/time_check
...
if [ "$?" = "0" ]; then
    ...
    if [[ $? -ne 0 ]] ; then
	echo "Run smooth_coverage.R failed cnv6" >&2
	## mysqladmin --socket=$BASE/thesock shutdown -u root
...
if [ "$?" = "0" ]; then
 ...
 echo "Run Get_3_random_ref_genes"
  mysqldump -u root --socket=$BASE/thesock --tab=$working_dir cnv6 cnv_sample_name_over_control_name_60bp_exon_ref1_med_gene_cov
  ...
  mysqldump -u root --socket=$BASE/thesock --tab=$working_dir cnv6 cnv_sample_name_over_control_name_60bp_exon_ref2_med_gene_cov
  ...
  mysqldump -u root --socket=$BASE/thesock --tab=$working_dir cnv6 cnv_sample_name_over_control_name_60bp_exon_ref3_med_gene_cov
...
grep "plot_genes_ordered_cnv6.py" $working_dir/completed.txt > /dev/null 2>&1
if [ "$?" = "0" ]; then
    echo "plot_genes_ordered_cnv6.py already run"
else
    echo "plot_genes_ordered_cnv6.py"
    #R CMD BATCH plot_genes_ordered.R
    python plot_genes_ordered_cnv6.py
    if [[ $? -ne 0 ]] ; then
	echo "Run plot_genes_ordered_cnv6.py failed" >&2
	## mysqladmin --socket=$BASE/thesock shutdown -u root
	#exit 1
    else
	echo "plot_genes_ordered_cnv6.py" >> $working_dir/completed.txt
    fi
fi
echo -n "Finished plot_genes_ordered_cnv6.py " >> $working_dir/time_check
timecheck=`(date +"%Y-%m-%d [ %H:%M:%S ]")`;
echo ${timecheck} >> $working_dir/time_check
...
then
	...
	head -n1 $working_dir/completed.txt > $working_dir/completed.txt.tmp
	mv $working_dir/completed.txt.tmp $working_dir/completed.txt
        echo "Empty cnv_calls file..RESTARTING cnv6"
	rm $working_dir/*_t	
exit 1
# do nothing as file is empty
fi
...
if [ "$?" = "0" ]; then
    ...
    if [[ $? -ne 0 ]] ; then
        ...
        echo "move_plots.sh cnv6" >> $working_dir/completed.txt
    fi
fi
